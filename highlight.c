...
static bool load_external_langs(const char *rule_config_path)
{
        ...
        if (sep) {
            *sep = 0;
            ...
            if (lang_path[0]) {
                ...
                if (strcmp(lang_path, "configs/uemacs/langs") == 0)
    ...
    if (!tried_repo_langs)
        loaded |= load_lang_dir("configs/uemacs/langs");
    return loaded;
}
...
void highlight_init(const char *rule_config_path)
{
    ...
    mystrscpy(global_config.colorscheme_name, "uemacs-dark", sizeof(global_config.colorscheme_name));
