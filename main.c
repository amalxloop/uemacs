...
#include "edef.h"
#include "version.h"
#include "ux.h"                  /* Added for uEmacs specific declarations */
...
int main(int argc, char **argv)
{
    ...
    nanox_init();            /* Initialize uEmacs system */
 ...
 loop:
    ...
    /* uEmacs help system handling */
    if (nanox_help_is_active()) {
        nanox_help_handle_key(c);
        goto loop;
    }

    /* Check for uEmacs global help key */
...
int execute(int c, int f, int n)
{
    ...
    TTbeep();
    nanox_set_lamp(NANOX_LAMP_WARN); /* uEmacs warning lamp */
