...
void nanox_get_user_data_dir(char *out, size_t cap) {
    ...
    if (local_app_data) {
        snprintf(out, cap, "%s\\uemacs", local_app_data);
    } else {
        const char *app_data = getenv("APPDATA");
        if (app_data) {
            snprintf(out, cap, "%s\\uemacs", app_data);
        } else {
            const char *user_profile = getenv("USERPROFILE");
            if (user_profile) {
                snprintf(out, cap, "%s\\.local\\share\\uemacs", user_profile);
            } else {
                snprintf(out, cap, "C:\\uemacs");
            }
        }
    }
#else
    ...
    if (xdg_data && *xdg_data) {
        snprintf(out, cap, "%s/uemacs", xdg_data);
    } else {
        ...
        if (home) {
            snprintf(out, cap, "%s/.local/share/uemacs", home);
        } else {
            snprintf(out, cap, "/tmp/uemacs");
        }
    }
#endif
}
...
void nanox_get_user_config_dir(char *out, size_t cap) {
    ...
    if (xdg_config && *xdg_config) {
        snprintf(out, cap, "%s/uemacs", xdg_config);
    } else {
        const char *home = getenv("HOME");
        if (!home) {
            struct passwd *pw = getpwuid(getuid());
        ...
        if (home) {
            snprintf(out, cap, "%s/.config/uemacs", home);
        } else {
            snprintf(out, cap, "/tmp/uemacs");
        }
    }
